...
$('.add-btn').on('click', function(e){
    ...
    $.ajax({
        type: "POST",
        url: basePath + "admin/add-partner",
        data: form_data,
...
var masterTable = $('#master-table').DataTable({
    ...
    "ajax": {
        url: basePath + 'admin/fetch-partners',
        type: "POST",
        dataType: 'json',
    ...
    'createdRow': function( row, data, dataIndex ) {
        $(row).attr('data-id', data.PartnerId);
    },
    columns: [
        {data: 'id'},
        {data: 'name'},
...
$('table#master-table').delegate('.view-record', 'click', function(){
    var PartnerId = $(this).parents('tr').attr('data-id');
    $.get(basePath+"admin/partners/"+PartnerId+"/edit", function(Partner){

        var form = $('form#edit-form');
        console.log(Partner);
        form.find('input[name="id"]').val(PartnerId);
        form.find('input[name="name"]').val(Partner.name);
        form.find('input[name="image"]').val(Partner.image);
        form.find('select[name="category_id"]').val(Partner.portfolio_id);
        ...
        if(Partner.image)
            {
                form.find('#holder img').attr('src',imagePath+Partner.image);
            }
...
$('.update-button').click(function(e){
    e.preventDefault();
    $('.modal-spinner').show();
    var PartnerId = $('form#edit-form').find('input[name="id"]').val();
    var form_data = $('form#edit-form').serializeArray();
    $.ajax({
        type: "POST",
        url: basePath + "admin/partner/"+ PartnerId,
        data: form_data,
        success: function (data) {
...
$('table#master-table').delegate('.delete-record', 'click', function(e){
    ...
    swal({
        ...
        if(isConfirm){
            $.ajax({
                url: basePath + 'admin/partners/' + PartnerId,
                type: 'post',
                data:{ id:PartnerId, _method: 'DELETE', _token: csrfToken
                },
                success: function(data){
                    if(data.status){
                        swal({
            ...
            swal("Not Deleted", "partner is not Deleted. it is save.", "error");
        }
    });
});
