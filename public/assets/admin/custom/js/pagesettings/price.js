...
$('.add-btn').on('click', function(e){
    ...
    $.ajax({
        type: "POST",
        url: basePath + "admin/add-price-plans",
        data: form_data,
...
var masterTable = $('#master-table').DataTable({
    ...
    "ajax": {
        url: basePath + 'admin/fetch-price-plans',
        type: "POST",
        dataType: 'json',
...
/*========== END SCRIPT TO ADD NEW Price =================*/

/*========== START SCRIPT TO EDIT AND VIEW Price =================*/
//edit 
$('#viewModal').find(".btn-edit").click(function (e) {
    e.preventDefault();
    if (confirm("Are you sure you want to edit this record ?")) {
        $("input,.form-control,select").prop("disabled", false);
        $(".btn-edit").hide("500");
...
$('table#master-table').delegate('.view-record', 'click', function(){
    var PriceId = $(this).parents('tr').attr('data-id');
    $.get(basePath+"admin/price-plan/"+PriceId+"/edit", function(Price){

        var form = $('form#edit-form');
        console.log(Price);
        form.find('input[name="id"]').val(PriceId);
        form.find('input[name="title"]').val(Price.title);
        form.find('input[name="price"]').val(Price.price);
        form.find('textarea[name="description"]').val(Price.description);
        if(Price.active_status){
...
/*========== END SCRIPT TO EDIT AND VIEW Price =================*/

/*========== START SCRIPT TO DELETE Price =================*/
$('table#master-table').delegate('.delete-record', 'click', function(e){
    ...
    swal({
        ...
        if(isConfirm){
            $.ajax({
                url: basePath + 'admin/price-plans/' + PriceId,
                type: 'post',
                data:{ id:PriceId, _method: 'DELETE', _token: csrfToken
                },
                success: function(data){
                    if(data.status){
                        swal({
            ...
            swal("Not Deleted", "Price is not Deleted. it is save.", "error");
        }
    });
});
/*========== END SCRIPT TO DELETE PriceS =================*/