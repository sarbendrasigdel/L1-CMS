...
$('.add-btn').on('click', function(e){
    ...
    $.ajax({
        type: "POST",
        url: basePath + "admin/add-portfolio-image",
        data: form_data,
...
var masterTable = $('#master-table').DataTable({
    ...
    "ajax": {
        url: basePath + 'admin/fetch-portfolio-images',
        type: "POST",
        dataType: 'json',
    ...
    'createdRow': function( row, data, dataIndex ) {
        $(row).attr('data-id', data.PortfolioImageId);
    },
    columns: [
        {data: 'id'},
        {data: 'project'},
...
$('table#master-table').delegate('.view-record', 'click', function(){
    var PortfolioImageId = $(this).parents('tr').attr('data-id');
    $.get(basePath+"admin/portfolio/"+PortfolioImageId+"/edit", function(portfolioImage){

        var form = $('form#edit-form');
        console.log(portfolioImage);
        form.find('input[name="id"]').val(PortfolioImageId);
        form.find('input[name="image"]').val(portfolioImage.image);
        form.find('select[name="category_id"]').val(portfolioImage.portfolio_id);


        if(portfolioImage.active_status){
            form.find('input[name="active_status"]').prop('checked', true);
        }
        if(portfolioImage.image)
            {
                form.find('#holder img').attr('src',imagePath+portfolioImage.image);
            }
...
$('.update-button').click(function(e){
    e.preventDefault();
    $('.modal-spinner').show();
    var PortfolioImageId = $('form#edit-form').find('input[name="id"]').val();
    var form_data = $('form#edit-form').serializeArray();
    $.ajax({
        type: "POST",
        url: basePath + "admin/portfolio-image/"+ PortfolioImageId,
        data: form_data,
        success: function (data) {
...
$('table#master-table').delegate('.delete-record', 'click', function(e){
    ...
    swal({
        ...
        if(isConfirm){
            $.ajax({
                url: basePath + 'admin/portfolio-images/' + PortfolioImageId,
                type: 'post',
                data:{ id:PortfolioImageId, _method: 'DELETE', _token: csrfToken
                },
                success: function(data){
                    if(data.status){
                        swal({
